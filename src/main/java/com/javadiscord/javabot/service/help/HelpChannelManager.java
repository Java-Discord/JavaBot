...
public class HelpChannelManager {
	...
	public static MessageEmbed getHelpChannelEmbed(String message, @Nullable Color color) {
	...
	public boolean mayUserReserveChannel(User user) {
		...
		if (member.getRoles().contains(Bot.config.get(this.config.getGuild()).getModeration().getMuteRole()))
			return false;
		try (var con = Bot.dataSource.getConnection();
			...
			e.printStackTrace();
			logChannel.sendMessageFormat("Error while checking if a user can reserve a help channel:\n```%s```", e.getMessage()).queue();
			return false;
		}
	}
	...
	public void reserve(TextChannel channel, User reservingUser, Message message) throws SQLException {
		...
		if (config.getReservedChannelMessage() != null) {
			message.replyEmbeds(
					getHelpChannelEmbed(
							this.config.getReservedChannelMessage(),
							this.config.getReservedColor())
			).queue();
		}
		...
		if (this.config.isRecycleChannels()) {
			...
			if (!dormantChannels.isEmpty()) {
				...
				targetChannel.sendMessageEmbeds(getHelpChannelEmbed(config.getReopenedChannelMessage(), config.getAvailableColor())).queue();
			} else {
	...
	private void unreserveChannelByOwner(TextChannel channel, User owner, Interaction interaction) {
		...
		getParticipantsSinceReserved(channel).thenAcceptAsync(participants -> {
			...
			if (potentialHelpers.isEmpty()) {
				Responses.info(interaction.getHook(), null, "Your channel has been unreserved.").queue();
	...
	private void sendThanksButtonsMessage(List<Member> potentialHelpers, ChannelReservation reservation, Interaction interaction, TextChannel channel) {
		...
		for (var helper : potentialHelpers.subList(0, Math.min(potentialHelpers.size(), 20))) {
			thanksButtons.add(new ButtonImpl("help-thank:" + reservation.getId() + ":" + helper.getId(), helper.getEffectiveName(), ButtonStyle.SECONDARY, false, Emoji.fromUnicode("❤")));
		}
		ActionRow controlsRow = ActionRow.of(
				Button.success("help-thank:" + reservation.getId() + ":done", "Unreserve"),
				Button.danger("help-thank:" + reservation.getId() + ":cancel", "Cancel")
		);
		List<ActionRow> rows = new ArrayList<>(5);
		rows.add(controlsRow);
		rows.addAll(MessageActionUtils.toActionRows(thanksButtons));
		interaction.getHook().sendMessageEmbeds(getHelpChannelEmbed(config.getThankMessage(), null)).addActionRows(rows).queue();
	}
	...
	private void unreserveChannelByOtherUser(TextChannel channel, User owner, @Nullable String reason, Interaction interaction) {
		if (reason != null) {// The user provided a reason, so check that it's legit, then send a DM to the owner.
			if (reason.isBlank() || reason.length() < 5) {
				...
				Responses.info(interaction.getHook(), null, "The channel has been unreserved.").queue();
				unreserveChannel(channel).queue();
			...
			try (var con = Bot.dataSource.getConnection();
				...
				return RestAction.allOf(
						...
						getOpenChannelCount() >= config.getPreferredOpenChannelCount()
								...
								channel.sendMessageEmbeds(getHelpChannelEmbed(config.getDormantChannelMessage(), config.getDormantColor())))

								: RestAction.allOf(channel.getManager().setParent(openCategory).sync(openCategory),
								channel.sendMessageEmbeds(getHelpChannelEmbed(config.getReopenedChannelMessage(), config.getAvailableColor())))
