...
public class ProfileCommand implements SlashCommandHandler {
	...
	private String getDescription(Member member) {
		...
		var warnings = new ModerationService(member.getJDA(), Bot.config.get(member.getGuild()).getModeration()).getWarns(member.getIdLong()).size();
		try (var con = Bot.dataSource.getConnection()) {
			var repo = new QuestionPointsRepository(con);
			var points = repo.getAccountByUserId(member.getIdLong()).getPoints();
			sb.append(String.format(
					"""
